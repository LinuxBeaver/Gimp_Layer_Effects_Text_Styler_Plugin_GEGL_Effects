...
enum_start (gegl_blend_mode_typeigcrop)
  ...
  enum_value (GEGL_BLEND_MODE_TYPE_GRAINMERGE,      "grainmerge",
              N_("Grain Merge"))
 enum_end (GeglBlendModeTypeig0crop)


property_enum (bevelblendmode, _("Emboss blend mode:"),
    GeglBlendModeTypeig0crop, gegl_blend_mode_typeigcrop,
...
#define GEGL_OP_META
#define GEGL_OP_NAME     inner_glow299
#define GEGL_OP_C_SOURCE inner-glow299.c
...
"    color-to-alpha color=black   transparency-threshold=0 opacity-threshold=1  "\
  ...
  state->plug    = gegl_node_new_child (gegl,
                                  "operation", "gegl:nop",
                                  NULL);
...
switch (o->mode) {
        break;
    case DEFAULT_IG:
 gegl_node_link_many (state->input, state->median2,  state->plug, state->it,  state->shadow, state->color, state->in, state->color2,  state->output, NULL);
 gegl_node_connect (state->in, "aux", state->plug, "output");
        break;
    case INVERT_TRANSPARENCY_IG:
  gegl_node_link_many (state->input, state->median2,  state->plug, state->it,  state->shadow,  state->it2, state->color, state->in,  state->color2, state->output, NULL);
        break;
    case DEFAULT_IG_IMAGE_UPLOAD:
 gegl_node_link_many (state->input, state->median2,  state->plug, state->it,  state->shadow, state->color, state->in, state->color2, state->idref, state->atop, state->output, NULL);
 ...
 gegl_node_link_many (state->input, state->median2, state->it,  state->shadow, state->it2,  state->color, state->in,  state->color2,  state->idref, state->atop, state->output, NULL);
 gegl_node_link_many (state->idref, state->image, state->blurimage,  NULL);
 gegl_node_connect (state->atop, "aux", state->blurimage, "output");
        break;
    case FEB_2024_IG:
 gegl_node_link_many (state->input, state->medianset, state->plug, state->idref, state->in2,  state->output, NULL);
...
/*This is median blur followed by GEGL's src-in blend mode and a crop*/
 gegl_node_link_many (state->input, state->medianset,  state->plug, state->idref3, state->in3,  state->output, NULL);
...
gegl_node_link_many  (state->idref3, state->it,  state->shadow,  state->color2, state->idref2, state->ingrainy,  NULL);
/*Inside the second src in blend mode with have the pick filter.*/
 ...
 gegl_node_link_many (state->idref2, state->pick, NULL);
        break;
    case BEVEL_IG:
 gegl_node_link_many (state->input, state->medianset, state->plug, state->idref, state->in4, state->output, NULL);
 gegl_node_link_many  (state->idref, state->it,  state->shadow, state->color2, usethis,  NULL);
...
GeglOperationMetaClass *operation_meta_class = GEGL_OPERATION_META_CLASS (klass);
  ...
  gegl_operation_class_set_keys (operation_class,
    "name",        "lb:innerglow299",
    "title",       _("Inner Glow (to blend) (no crash 2.99.19)"),
    ...
    "gimp:menu-label", _("Inner Glow (to blend) 2.99.19 friendly..."),
    NULL);
}
