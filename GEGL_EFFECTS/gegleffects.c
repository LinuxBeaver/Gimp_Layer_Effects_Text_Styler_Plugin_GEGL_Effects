...
property_double  (fixoutline, _("Fix Inner Glow outline with median blur"), 80)
  value_range (50, 100)
...
static void attach (GeglOperation *operation)
{
  ...
  GeglNode *input, *output, *image, *atop, *mbd, *nopig, *multiplyb, *nopm, *over, *multiply, *nopb, *mcol, *stroke, *innerglow, *gradient, *crop, *ds;
  ...
  gegl_node_link_many (input, atop, crop, nopb, multiplyb, nopm, multiply, nopig, over, stroke, ds, output, NULL);
...
update_graph (GeglOperation *operation)
{
  ...
  if (o->innerglow)
  {
    if (o->gradient)
    {
      ...
      gegl_node_link_many (state->input, state->atop, state->crop, state->nopb, state->multiplyb, state->nopm, state->multiply, state->nopig, state->over, state->stroke, state->ds, state->output, NULL);
      ...
      gegl_node_connect_from (state->multiplyb, "aux", state->mbd, "output");
  ...
  else
  {
    if (o->gradient)
    {
      ...
      gegl_node_link_many (state->input, state->atop, state->crop, state->nopb, state->multiplyb, state->nopm, state->multiply, state->stroke, state->ds, state->output, NULL);
      gegl_node_link_many (state->input, state->image, state->gradient, NULL);
      gegl_node_link_many (state->nopb, state->mbd, NULL);
      gegl_node_link_many (state->nopm, state->mcol, NULL);
      gegl_node_connect_from (state->multiplyb, "aux", state->mbd, "output");
