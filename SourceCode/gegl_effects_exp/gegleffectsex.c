...
In Example, Gimpꞌs Split blend mode is 60

split = gegl_node_new_child (gegl,
"operation", "gimp:layer-mode", "layer-mode", 60,
"composite-mode", 0, "blend-space", 0,  NULL);

If Gimpꞌs team adds a new blend mode this will throw off all the gimp exclusive blend modes called by one number. 

Resulting in a scenario where all my plugins will not work (by using the wrong blend mode) until the number 60 is changed to 61 or 59. (weꞌll have to figure it out when it happens) but Iꞌll guess 61.
...
--
The second most common reason GEGL Effects will break is if a GEGL operation changes its properties. In example gaussian blurꞌs values are
...
hypothetical scenario would be  if gegl:emboss got renamed to gegl:embosser; weꞌd have to udpate all my filters that call emboss.

The fourth most common scenario is if Gimpꞌs team adds a new filter that has the exact same name as one of mine in both the title and operation name. 
...
Crude recreation of parts of GEGL Effects graph. This just shows how the nodes are ordered. They are many things missing and I canꞌt possibly list all the options. 
...
enum_start (gegl_blend_mode_type_effectszzbevoutlinege_ex)
...
enum_end (GeglBlendModeTypezzbevoutlinege_ex)
...
enum_start (gegl_blend_mode_type_effectszzbevoutlinegeextra_ex)
  ...
  enum_value (GEGL_BLEND_MODE_TYPE_DISABLEBEVELGEextra,      "DisableBevelGE",
              N_("Outline Bevel Off"))
enum_end (GeglBlendModeTypezzbevoutlinegeextra_ex)


/*This is the enum list of blend modes for bevel.*/
enum_start (gegl_blend_mode_type_effectszz_ex)
  ...
  enum_value (GEGL_BLEND_MODE_TYPE_BEVELOFF,      "BevelOff",
              N_("Bevel off "))
enum_end (GeglBlendModeTypezz_ex)



/*This is the enum list of blend modes for Inner Glow.*/
enum_start (gegl_blend_mode_type_effectsigzz_ex)
  ...
  enum_value (GEGL_BLEND_MODE_TYPE_HUEIG,      "Hue",
              N_("HSV Hue"))
enum_end (GeglBlendModeTypeigzz_ex)

/*This is the enum list of blend modes for Gradient.*/
enum_start (gegl_blend_mode_type_effectsgzz_ex)
  ...
  enum_value (GEGL_BLEND_MODE_TYPE_LCHCOLORG,      "LCHColor",
              N_("LCh Color"))
enum_end (GeglBlendModeTypegzz_ex)

/*This is the enum list of blend modes for Image File Overlay.*/
enum_start (gegl_blend_mode_type_effectsgzzimage_ex)
  ...
  enum_value (GEGL_BLEND_MODE_TYPE_HSVHUEIMAGE,      "HSVHue",
              N_("HSV Hue"))
enum_end (GeglBlendModeTypegzzimage_ex)

/*This is the enum list of blend modes for Color Overlay*/
enum_start (gegl_blend_mode_type_effectsgzzcolor_ex)
  ...
  enum_value (GEGL_BLEND_MODE_TYPE_NOCOLOR,      "NoColor",
              N_("No Color"))
enum_end (GeglBlendModeTypegzzcolor_ex)

/* This is the enum list of outlineꞌs base shape. Should correspond to GeglMedianBlurNeighborhood in median-blur.c */
enum_start (gegl_stroke_grow_shapeszz_ex)
...
enum_end (GeglstrokeGrowShapeszz_ex)

/* This is the enum list of extra outlineꞌs base shape. Should correspond to GeglMedianBlurNeighborhood in median-blur.c */
enum_start (gegl_stroke_grow_shapeszzextra_ex)
...
enum_end (GeglstrokeGrowShapeszzextra_ex)

/*This is the enum list of shiny textꞌs blend mode switcher.*/
enum_start (gegl_blend_mode_typeshinegeffects_ex)
  ...
  enum_value (GEGL_BLEND_MODE_TYPE_REPLACESHINE, "Replace",
              N_("Multiply"))
enum_end (GeglBlendModeTypeshinegeffects_ex)


/*This is the enum list of glass textꞌs blend mode switcher.*/
enum_start (gegl_blend_mode_type_glass_ex)
  ...
  enum_value (GEGL_BLEND_MODE_TYPE_OVERLAYGLASS, "Overlay",
              N_("Overlay"))
enum_end (geglblendmodetypeglass_ex)


/*This is the enum list for parts of GEGL Effects to be displayed (part of filter to be displayed)*/
enum_start (guichangeenumzz_ex)
enum_value   (BEAVER_UI_STROKESHADOW, "strokeshadow", N_("Color Fill, Outline and Shadow (Basic)"))
enum_value   (BEAVER_UI_INNERGLOWBEVEL, "innerglowbevel", N_("Bevel and Inner Glow (Basic)"))
enum_value   (BEAVER_UI_IMAGEGRADIENT, "imagegradient", N_("Image file upload and Gradient (Basic)"))
enum_value   (BEAVER_UI_OUTLINESPECIAL, "outlinespecial", N_("Special Options for Outline and Shadow (Advance)"))
enum_value   (BEAVER_UI_MISCOPTIONS, "miscoptions", N_("Miscellaneous Text Effects (Advance)"))
enum_value   (BEAVER_UI_EXTRAOSG, "extraosg", N_("Add a extra (second) Outline Shadow Glow (Advance)"))
enum_value   (BEAVER_UI_GLASSTEXTPARTNER, "glasstextpartner", N_("Glass Text and GEGL Syntax (Advance)"))
enum_value   (BEAVER_UI_AMAZING, "amazing", N_("Alt Text Styles and Extrusions (Advance)"))
  enum_end (guiendzz_ex)

/*Most major text styling plugins are here*/
enum_start (my_major_plugins_ge_ex)
  ...
  enum_value (GEGL_CHROME_TIME, "chrome",
              N_("Chrome"))
enum_end (mymajorpluginsge_ex)

/*Blend mode list for major text styling plugins Custom Bevel and Sharp Bevel are here*/


enum_start (gegl_blend_mode_typecbsharp_ge_ex)
  enum_value (GEGL_BLEND_MODE_TYPE_HARDLIGHT_AMAZING, "Hardlight",
              N_("HardLight"))
  enum_value (GEGL_BLEND_MODE_TYPE_MULTIPLY_AMAZING,      "Multiply",
              N_("Multiply"))
  enum_value (GEGL_BLEND_MODE_TYPE_COLORDODGE_AMAZING,      "ColorDodge",
              N_("ColorDodge"))
  enum_value (GEGL_BLEND_MODE_TYPE_PLUS_AMAZING,      "Plus",
              N_("Plus"))
  enum_value (GEGL_BLEND_MODE_TYPE_DARKEN_AMAZING,      "Darken",
              N_("Darken"))
  enum_value (GEGL_BLEND_MODE_TYPE_LIGHTEN_AMAZING,      "Lighten",
              N_("Lighten"))
  enum_value (GEGL_BLEND_MODE_TYPE_OVERLAY_AMAZING,      "Overlay",
              N_("Overlay"))
  enum_value (GEGL_BLEND_MODE_TYPE_GRAINMERGE_AMAZING,      "GrainMerge",
              N_("Grain Merge"))
  enum_value (GEGL_BLEND_MODE_TYPE_SOFTLIGHT_AMAZING,      "Softlight",
              N_("Soft Light"))
  enum_value (GEGL_BLEND_MODE_TYPE_ADDITION_AMAZING,      "Addition",
              N_("Addition"))
  enum_value (GEGL_BLEND_MODE_TYPE_EMBOSSBLEND_MAJOR,      "EmbossBlend",
              N_("Grayscale Multiply for Image uploads"))
enum_end (GeglBlendModeTypecbsharpge_ex)
...
property_enum(guichange, _("Part of filter to be displayed"),
    guiendzz_ex, guichangeenumzz_ex,
...
property_color  (optioncolor, _("Text color change (default only works on white text.)"), "#ffffff")
  ...
  ui_meta     ("sensitive", "! enableamazing")



property_enum (colorblendmode, _("Blend Mode of Color Overlay"),
    GeglBlendModeTypegzzcolor_ex, gegl_blend_mode_type_effectsgzzcolor_ex,
...
and it saves space in the GUI. If one day a better GUI comes along Iꞌd like to enable this. */

property_boolean (enableoutline, _("Enable Outline"), FALSE)
  description    (_("Disable or Enable Outline"))
  ui_meta ("visible", "guichange {strokeshadow}")

property_double (opacitystroke, _("Outlineꞌs Opacity"), 1.0)
  value_range   (0.0, 1.0)
  ui_steps      (0.01, 0.50)
  ui_meta ("visible", "guichange {strokeshadow}")


property_enum   (grow_shape, _("Outlineꞌs Base Shape"),
                 GeglstrokeGrowShapeszz_ex, gegl_stroke_grow_shapeszz_ex,
...
property_double (grow_radiusstroke, _("Outlineꞌs Size"), 12.0)
...
property_color  (colorstroke, _("Outlineꞌs Color"), "#000000")
  description   (_("The outlineꞌs color"))
...
property_double (slideupblack, _("Black Bevel and Image Bevel mode. "), 0.00)
    description (_("When in use Bevel works correctly on black Bevels when using blend modes like Grain Merge and Hardlight. All you have to do is select those blend modes for black text and then move this slider up. This same slider can also be used so bevel can apply to image file overlayꞌs while ignoring their content."))
...
property_double (bevelopacity, _("Bevelꞌs Opacity"), 1.00)
  value_range   (0.15, 1.00)
  ui_steps      (0.01, 0.50)
  ui_meta ("visible", "guichange {innerglowbevel}")
  ui_meta     ("sensitive", "! enableamazing")
...
property_enum (typebevel, _("Type of Bevel"),
    GeglBlendModeTypebeavbeveleffects_ex, gegl_blend_mode_typebeavbeveleffects_ex,
    GEGL_BEVEL_NORMAL)
...
enum_start (gegl_blend_mode_typebeavbeveleffects_ex)
  enum_value (GEGL_BEVEL_NORMAL,      "normalbevel",
              N_("Bump Bevel"))
  enum_value (GEGL_BEVEL_SHARP,      "sharpbevel",
              N_("Sharp Bevel"))
enum_end (GeglBlendModeTypebeavbeveleffects_ex)


property_enum (blendmodebevel2, _("Select blend or Enable/Disable Bevel"),
    GeglBlendModeTypezz_ex, gegl_blend_mode_type_effectszz_ex,
...
property_enum (blendmodeinnerglow2, _("Blend Mode of Inner Glow"),
    GeglBlendModeTypeigzz_ex, gegl_blend_mode_type_effectsigzz_ex,
    GEGL_BLEND_MODE_TYPE_OVER)
  ui_meta ("visible", "guichange {innerglowbevel}")

property_double (innergradius, _("Inner Glowꞌs Blur intensity"), 4.5)
...
property_double (innerggrow_radius, _("Inner Glowꞌs Size"), 5)
  value_range   (1, 40.0)
  ui_range      (1, 30.0)
  ui_digits     (0)
  ui_steps      (1, 5)
...
property_double (innergopacity, _("Inner Glowꞌs Opacity"), 1.4)
  value_range   (0.0, 2.0)
  ui_steps      (0.01, 0.10)
  ui_meta ("visible", "guichange {innerglowbevel}")

property_color (innergvalue, _("Inner Glowꞌs Color"), "#ff8f00")
...
property_double  (fixoutline, _("Inner Glowꞌs unmodified pixel fix"), 75)
  value_range (50, 85)
  description (_("If innerglow isnꞌt covering a few pixels on the edge. Slide this up.ꞌ"))
  ui_meta ("visible", "guichange {innerglowbevel}")
...
property_enum (imageblendmode, _("Blend Mode of Image File Overlay (For layer content)"),
    GeglBlendModeTypegzzimage_ex, gegl_blend_mode_type_effectsgzzimage_ex,
...
property_enum (blendmodegradient2, _("Blend Mode of Gradient"),
    GeglBlendModeTypegzz_ex, gegl_blend_mode_type_effectsgzz_ex,
    GEGL_BLEND_MODE_TYPE_OVER)
  ui_meta ("visible", "guichange {imagegradient}")
  ui_meta     ("sensitive", "! enableamazing")
...
property_enum (blendmodebeveloutline, _("Select blend or Enable/Disable Outline Bevel"),
    GeglBlendModeTypezzbevoutlinege_ex, gegl_blend_mode_type_effectszzbevoutlinege_ex,
...
property_double (osbevelopacity, _("Outline Bevelꞌs Opacity"), 1.0)
...
property_double (oshue, _("Hue rotation of Outlineꞌs Image file overlay"),  0.0)
   description  (_("Hue adjustment"))
   value_range  (-180.0, 180.0)
  ui_meta ("visible", "guichange {outlinespecial}")


property_double (oslightness, _("Lightness of Outlineꞌs Image file overlay"), 0.0)
   description  (_("Lightness adjustment"))
   value_range  (-100.0, 100.0)
  ui_meta ("visible", "guichange {outlinespecial}")

property_boolean (specialshadowoptions, _("Enable/Disable special options for Shadow/Glow"), FALSE)
  description    (_("Turn on special drop shadow glowꞌs special abilities"))
  ui_meta ("visible", "guichange {outlinespecial}")
...
property_double (blurshadowimage, _("Blur Shadow/Glowꞌs Image file overlay"), 0)
...
property_seed (seedaura, _("Seed of Glow Aura"), randaura)
  ui_meta ("visible", "guichange {outlinespecial}")
/* Two rand commands canꞌt co-exist but if it is renamed to "randaura" it appears to work just fine */
...
property_enum (blendmodeshine, _("Blend Mode of Shiny Text"),
    GeglBlendModeTypeshinegeffects_ex, gegl_blend_mode_typeshinegeffects_ex,
...
property_double (complexity, _("Complexity of Gloss Shine"), 1.0)
...
property_double (opacitystrokeextraoutline, _("Extra Outline/Shadow/Glowꞌs Opacity"), 1.0)
  value_range   (0.0, 1.0)
  ui_steps      (0.01, 0.50)
  ui_meta ("visible", "guichange {extraosg}")
...
property_enum   (grow_shapeextraoutline, _("Extra Outline/Shadow/Glowꞌs Base Shape"),
                 GeglstrokeGrowShapeszzextra_ex, gegl_stroke_grow_shapeszzextra_ex,
...
property_double (grow_radiusstrokeextraoutline, _("Extra Outlineꞌs Size (must be past default outline)"), 19.0)
  value_range   (0.0, 100.0)
  ui_range      (0.0, 100.0)
  ui_digits     (0)
  ui_steps      (1, 5)
...
property_color  (colorstrokeextraoutline, _("Extra Outline/Shadow/Glowꞌs Color"), "#2400ff")
  description   (_("The outlineꞌs color"))
  ui_meta ("visible", "guichange {extraosg}")
...
property_enum (blendmodebeveloutlineextra, _("Select blend or Enable/Disable Extra Outline Bevel"),
    GeglBlendModeTypezzbevoutlinegeextra_ex, gegl_blend_mode_type_effectszzbevoutlinegeextra_ex,
    GEGL_BLEND_MODE_TYPE_MULTIPLYGEextra)
  ui_meta ("visible", "guichange {extraosg}")

property_int (osdepthextra, _("Extra Outlineꞌs Bevel Depth"), 12)
    description (_("Depth of Outline Bevel that works with some but not all blend modes"))
    value_range (1, 70)
  ui_meta ("visible", "guichange {extraosg}")
...
property_double (osbevelopacityextra, _("Extra Outline Bevelꞌs Opacity"), 1.0)
  value_range   (0.15, 1.0)
  ui_steps      (0.01, 0.10)
  ui_meta ("visible", "guichange {extraosg}")
...
property_enum (glassblendmode, _("Blend Mode of Glass on Text"),
 geglblendmodetypeglass_ex, gegl_blend_mode_type_glass_ex,
...
/* Beginning of GEGL Graph */
property_string (graphstring, _(""), ANYTHINGGOESHERE)
  ui_meta ("visible", "guichange {glasstextpartner}")
...
/* Beginning of Amazing Text Styling Plugins */
property_boolean (enableamazing, _("Enable major text stylers (cancels out many settings)"), FALSE)
  description    (_("Most major GEGL text styling plugins are here. Enabling major text styling plugins in GEGL Effects will disable many other options. So if you are not using the major text styling plugins keep this checkbox disabled. Major Text Styling plugins work by calling all major text styling plugins that happen to share the same universal sliders (emboss, blur, median). However, not all of them share the same sliders, so a NA(NOT AVAILABLE) warning or something like it is present to explain to the user that a selected plugin doesnꞌt have a option. In Example, NA Glossy Balloon means that the slider wonꞌt work on Glossy Balloon."))
 ui_meta ("visible", "guichange {amazing}")

property_enum (amazinglist, _("Select a Major Text Plugin"),
    mymajorpluginsge_ex, my_major_plugins_ge_ex,
    GEGL_GLOSSY_BALLOON_TIME)
  ui_meta ("visible", "guichange {amazing}")


property_double (universalgaussian, _("Internal Gaussian Size (NA SharpBevel, Rock)"), 6.0)
...
property_int (universaldepth, _("Depth (NA GlossyBalloon, Rock, Chrome) "), 36)
...
property_int  (universalmedianradius, _("Median Radius (NA GlossyBalloon, Chrome)"), 3)
  value_range (-10, 20)
  ui_range    (-10, 20)
  ui_meta     ("unit", "pixel-distance")
...
property_double  (universalmedianalphapercentile, _("Median AP (NA GlossyBalloon, Chrome, SharpBevel)"), 75)
  value_range (21, 100)
...
property_color  (universalcolor, _("Color Selection (white is usually transparency)"), "#00c8ff")
    description (_("Universal Color Selection for all major text styling plugins that use it. Text styling plugins that do not use this filter will have no effect when this slider is used. They are listed under NA. On many plugins white equals transparency. If white doesnꞌt give transparency left click the color changer and move its A slider to 0."))
  ui_meta ("visible", "guichange {amazing}")

property_file_path(universalimage, _("Image Upload (NA GMM, Chrome)"), "")
...
property_enum (universalblendmode, _("Blend CustomBevel, SharpBevel, GMM (NA others)"),
    GeglBlendModeTypecbsharpge_ex, gegl_blend_mode_typecbsharp_ge_ex,
    GEGL_BLEND_MODE_TYPE_HARDLIGHT_AMAZING)
...
/* End of Amazing Text Styling Plugins */

/*  Start of long shadow extrusion plugins */
property_boolean (lspolicy, _("Enable/Disable Extrusions"), FALSE)
  description    (_("Enable Long Shadow and Long Shadow Pixel Data here."))
  ui_meta ("visible", "guichange {amazing}")

enum_start (long_shadow_type_ge)
...
enum_end (longshadowtypege)


property_enum (lstype, _("Extrusion Type"),
    longshadowtypege, long_shadow_type_ge,
    TYPE_LSPD)
  ui_meta ("visible", "guichange {amazing}")

property_double (lsangle, _("Angle of Extrusion"), 45.0)
...
property_double (lslength, _("Length of Extrusion"), 40)
...
property_double (lslengthblur, _("Length of Pixel Data colors (PD Only)"), 100.0)
...
property_double (lsmidpoint, _("Midpoint (FFL Only)"), 0.4)
  description (_("Shadow fade midpoint. On non FFL (Fading Fixed Length) mode this will do nothing."))
  value_range (0.0, 1.0)
  ui_range    (0.0, 1.0)
  ui_meta ("visible", "guichange {amazing}")

property_color (lscolor, _("Color (FFL and Normal only)"), "black")
  description (_("Shadow color. This will not work in Pixel Data Mode."))
  ui_meta ("visible", "guichange {amazing}")
/*  End of long shadow extrusion plugins */


#else

#define GEGL_OP_META
#define GEGL_OP_NAME     gegleffectsex
#define GEGL_OP_C_SOURCE gegleffectsex.c
...
/*All nodes that have no relationships (shiny text,  microblur, thin to thick text, extrusion, and amazing) start here*/
  GeglNode *shiny;
  GeglNode *microblur;
  GeglNode *thinbold;
  ...
  GeglNode *amazing;
  GeglNode *extrusions;
  GeglNode *anythinggraph;
/*All nodes that have no relationships (shiny text, microblur, thin to thick text, extrusion, and amazing) end here*/
  ...
  if (o->innerglow)
  {
    if (o->gradient)
    {
      /* both innerglow and gradient */
         gegl_node_link_many (state->input, state->thinbold, state->microblur, state->nopimage, atopi,  multiply, state->cropcolor,  state->nopg, atopg,  state->crop, state->shiny, state->nopb, bevelmode, state->cropigb, state->amazing, state->nopglass,  state->glassover,   state->nopextrassg, state->knockoutidref,  state->nopig, over,  state->nopstrokebehind, state->strokebehind, state->behindextrassg,  state->extrusions, state->nopdsbehind, state->dsbehind, state->xor, state->repairgeglgraph, state->anythinggraph,  state->output, NULL);
      ...
      gegl_node_connect (atopi, "aux", state->saturationvalue, "output");
      ...
      /*inner glow leg clop fix due to bevelꞌs dst*/
      gegl_node_connect (state->cropigb, "aux", state->nopb, "output");
    }
    else
    {
      /* innerglow but no gradient */
         gegl_node_link_many (state->input, state->thinbold, state->microblur, state->nopimage, atopi, multiply, state->crop,  state->shiny, state->nopb, bevelmode, state->cropigb, state->amazing, state->nopglass, state->glassover,   state->nopextrassg, state->knockoutidref, state->nopig, over,  state->nopstrokebehind, state->strokebehind,  state->behindextrassg, state->extrusions, state->nopdsbehind, state->dsbehind, state->xor,  state->repairgeglgraph, state->anythinggraph, state->output, NULL);
      /* Nodes relating to color overlay */
      gegl_node_link_many (state->mcol, state->coloropacity, NULL);
      gegl_node_connect (multiply, "aux", state->coloropacity, "output");
      /* Nodes relating to knockout text */
  ...
  else
  {
    if (o->gradient)
    {
      /* gradient but no innerglow */
         gegl_node_link_many (state->input, state->thinbold, state->microblur,  state->nopimage, atopi,   multiply, state->cropcolor,  state->nopg, atopg, state->crop, state->shiny, state->nopb, bevelmode, state->cropigb, state->amazing, state->nopglass,  state->glassover,  state->nopextrassg, state->knockoutidref,  state->nopstrokebehind, state->strokebehind, state->behindextrassg, state->extrusions, state->nopdsbehind, state->dsbehind, state->xor,  state->repairgeglgraph, state->anythinggraph, state->output, NULL);
      /* Nodes relating to color overlay */
      gegl_node_link_many (state->mcol, state->coloropacity, NULL);
      gegl_node_connect (multiply, "aux", state->coloropacity, "output");
      /* Nodes relating to knockout text */
...
/* All nodes relating to outline begin here.
  ...
  zzstrokebevelimage is a hidden operation; recreation of gimpꞌs drop shadow filter with heavy modification so it can do things like bevel outlines, aura seeds and image file overlays.
gegl effects use to use the default drop shadow until Beaver realized that the only way to give GEGL Effects styles seen in Adobeꞌs layer effects was to abandon gegl:dropshadow and
...
xor = gegl_node_new_child (gegl,
  ...
  /*The nop functions as a ref and ID for color overlayꞌs placement in a GEGL Graph*/
  nopm = gegl_node_new_child (gegl,
                                  "operation", "gegl:nop",
                                  NULL);


  /*gegl:cropꞌs are applied after gegl:color or anything gegl renders to prevent a known bug. */
...
/* All nodes relating to the bevel start here*/
  ...
  mbd = gegl_node_new_child (gegl,
                                  ...
                                  NULL);
 /*The Threshold Alpha setting of the bevel is being baked in so it isnꞌt present in the GUI.*/

  mbdopacity = gegl_node_new_child (gegl,
                                  "operation", "gegl:opacity",
                                  NULL);

  /*The nop functions as a ref and ID for bevelꞌs placement in a GEGL Graph*/
...
/* DST doesnꞌt disable bevel it just makes it invisible*/
...
/* All nodes relating to Inner Glow start here*/
  ...
  /*The nop functions as a ref and ID for inner glowꞌs placement in a GEGL Graph*/
...
/* Blend modes for Inner Glow are here*/
  ...
  /*over is the switch for for gegl:innerglowꞌs blend mode switcher*/
...
/* All nodes relating to Gradientꞌs start here*/
  ...
  /*To prevent a known bug gegl:cropꞌs are used after anything gegl renders; such as a linear gradient*/
  ...
  /*The nop functions as a ref and ID for gradientꞌs placement in a GEGL Graph*/
...
/* All nodes relating to Gradientꞌs end here*/
...
/* All nodes relating to image file overlay begin here*/
  ...
  /*The nop functions as a ref and ID for image file overlayꞌs placement in a GEGL Graph*/
...
/* All nodes for image file overlay end here*/
  ...
  extrusions = gegl_node_new_child (gegl,
                                  "operation", "lb:ls", "lightness", -7.0,
                                  NULL);

 anythinggraph   = gegl_node_new_child (gegl,
                                  "operation", "gegl:gegl", 
                                  NULL);

  amazing = gegl_node_new_child (gegl,
                                  "operation", "lb:amazing",  /*This list all major text styling plugins*/
                                  NULL);
  ...
  cropigb      = gegl_node_new_child (gegl, "operation", "gegl:crop",
                                         NULL);
...
drop shadow is applied in a gegl graph below them.*/
 ...
 /* All nodes relating to glass on text end here */
  ...
  /*Beginning of Shadowꞌs GUI asociations*/
  ...
  /*End of Shadowꞌs GUI asociations*/

  /*Beginning of Outlineꞌs GUI asociations*/
  ...
  /*End of Outlineꞌs GUI asociations*/

  /*Beginning of Bevelꞌs GUI asociations*/
  ...
  /*End of Bevelꞌs GUI asociations*/

  /*Beginning of Image file Overlayꞌs GUI asociations*/
  ...
  /*End of of Image file Overlayꞌs GUI asociations*/

  /*Beginning of Inner Glowꞌs GUI asociations*/
  ...
  /*End of Inner Glowꞌs GUI asociations*/
  

  /*Beginning of Color Overlayꞌs GUI asociations*/
  ...
  /*End of Color Overlayꞌs GUI asociations*/

  /*Beginning of Gradientꞌs GUI asociations*/
  ...
  /*End of Gradientꞌs GUI asociations*/

  /*Beginning of Shiny Textꞌs GUI asociations*/
  ...
  /*End of Shiny Textꞌs GUI asociations*/

  /*Beginning of Micro Blurꞌs GUI asociations*/  
  ...
  /*End of Micro Blurꞌs GUI asociations*/  
  ...
  /*Beginning of Extra Outline Shadow Glowꞌs GUI asociations*/
  ...
  /*End of Extra Outline Shadow Glowꞌs GUI asociations*/

  /*Beginning of Knock Out Textꞌs GUI asociations*/
  gegl_operation_meta_redirect (operation, "knockout", opacityinput, "value"); 
  /*End of Knock Out Textꞌs GUI asociations*/

  /*Beginning of GEGL Graphꞌs GUI asociations*/
  gegl_operation_meta_redirect (operation, "graphstring", anythinggraph, "string"); 
  /*End of GEGL Graphꞌs GUI asociations*/


  /*Beginning of Glass Over Textꞌs GUI asociations*/
  ...
  /*Beginning of End of Glass Over Textꞌs GUI asociations*/

  /*Beginning of Amazing Text Styling Pluginꞌs GUI asociations*/
  gegl_operation_meta_redirect (operation, "enableamazing", amazing, "policy");
  gegl_operation_meta_redirect (operation, "amazinglist", amazing, "list");
  gegl_operation_meta_redirect (operation, "universalgaussian", amazing, "universalgaussian");
  gegl_operation_meta_redirect (operation, "universalmedianradius", amazing, "universalmedianradius");
  gegl_operation_meta_redirect (operation, "universalmedianalphapercentile", amazing, "universalmedianalphapercentile");
  gegl_operation_meta_redirect (operation, "universalcolor", amazing, "universalcolor");
  gegl_operation_meta_redirect (operation, "universalazimuth", amazing, "universalazimuth");
  gegl_operation_meta_redirect (operation, "universalelevation", amazing, "universalelevation");
  gegl_operation_meta_redirect (operation, "universaldepth", amazing, "universaldepth");
  gegl_operation_meta_redirect (operation, "universalimage", amazing, "universalimage");
  gegl_operation_meta_redirect (operation, "universalblendmode", amazing, "universalblendmode");
  /*End of Amazing Text Styling Pluginꞌs GUI asociations*/


  /*Start of Long Shadow Extrusion GUI asociations*/
  gegl_operation_meta_redirect (operation, "lspolicy", extrusions, "longshadowpolicy");
  gegl_operation_meta_redirect (operation, "lstype", extrusions, "longshadowtype");
  ...
  /*All Nodes with no relationships (shiny text, micro blur, thin to thick slider, extrusions, amazing, ) start here*/
  state->shiny = shiny;
  state->microblur = microblur;
  state->thinbold = thinbold;
  state->repairgeglgraph = repairgeglgraph;
  state->cropigb = cropigb;
  state->amazing = amazing;
  state->extrusions = extrusions;
  state->anythinggraph = anythinggraph;
  /*All Nodes with no relationships (shiny text, micro blur, thin to thick slider, extrusions amazing) end here*/
...
gegl_op_class_init (GeglOpClass *klass)
{
 ...
 /*btw, if GEGL Effects (or any of my plugins) ever breaks try changing the name space from gegl: or lb: to something else.*/
  gegl_operation_class_set_keys (operation_class,
    ...
    "gimp:menu-label", _("GEGL Effects Experimental..."),
    NULL);
}
