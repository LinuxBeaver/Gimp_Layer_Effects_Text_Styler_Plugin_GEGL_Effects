...
/*On August 6th 2023 Beaver found out these hidden GUI properties below couldnꞌt be embedded into a GEGL Operationꞌs property listing. If you look at lb:gmmꞌs GEGL node listing you will notice that many properties are already embedded. (12 exactly) It doesnꞌt allow anymore, and if I did add a 13th one Gimp 2.10 would crash. We believe a GEGL operations property list caps out at 12 settings or when it reaches a certain point in memory. The problem may be caused by either or hypothesis, or something else. Due to this, Beaver is forced to use "output-extent" to hide additional baked in settings*/
...
/*On August 6th 2023 Beaver learned that the operations property list as a memory or number cap. It doesnꞌt allow more then 12-13 numbers in a list.

The goal is to make all three GMMꞌs true to the style of Glass Metal and Marble. The first is Glass, The second is Metal, the third is Marble.*/
...
/*There is no way to embed these operations to hide them. GMMꞌs operation properties has reached its limit.*/
...
/*fun fact, rock text in GEGL Effects has a edge smooth that isnꞌt present in the default rock text filter
